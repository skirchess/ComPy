...
def bracketing(f,a,b): #a<b
    ...
    if f(a)*f(b)>0: #else  #f(a) and f(b) have same sign
        if abs(f(a))<abs(f(b)):
            a=a-1.5*(b-a)   # shifting a to left
            count+=1
            return bracketing(f,a,b)
        if abs(f(a))>abs(f(b)): 
            b=b+1.5*(b-a)  # shifting b to right
            count+=1
            return bracketing(f,a,b)
...
def regula(f,a,b,E,D):
    ...
    if f(a)*f(b)<0:
        if abs(b-a)>E or (f(a)>D and f(b)>D):
            while abs(a-b)>E:
                if f(a)*f(c)<0:
                    b=c
                if f(b)*f(c)<0:
        ...
        print('Choose different interval')
